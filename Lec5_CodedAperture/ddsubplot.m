...
isHGUsingMATLABClasses = 1;%feature('HGUsingMATLABClasses');
...
% inset = [.2 .18 .04 .1]; % [left bottom right top] -- stock MATLAB, wasteful
% inset = 0.1.*[.15 .0 0 .1]; % [left bottom right top] -- too tight, titles clip
inset = [.10 .14 .02 .09]; % [left bottom right top] -- interior gutters
...
if ~isempty(h) && ~move_axis
    ...
    if isempty(position)
        ...
        elseif max(thisPlot) > ncols*nrows
            ...
            % Outer margins of the whole grid, [left bottom width height].
            % The top of the topmost axes lands at def_pos(2)+def_pos(4),
            % so the reserve for the top row's title lives here, not in
            % inset.  Was [0,0,0.99,0.99], which left no title room at all.
            def_pos = [0.07, 0.07, 0.91, 0.86];
...
if isempty(p) || ~ishghandle(p)
    % CLF detaches the axes from the figure before this listener fires,
    % so Parent can come back empty.  There is no grid left to clean up.
    % Without this guard, get([],'BeingDeleted') returns [] rather than
    % erroring, the strcmp below is false, and isappdata([]) throws.
    return
end
